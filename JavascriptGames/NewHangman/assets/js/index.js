window.onload = function() {
	...
	function select() {
		if (chosenCategory === randomWord[0]) {
			categoryName.innerHTML = "Category: Alien";
		} else if (chosenCategory === randomWord[1]) {
			categoryName.innerHTML = "Category: Human";
		}
	}
	...
	function life() {
		...
		if (tries < 1) {
			...
			displayTries.innerHTML = "Game Over, Jerry";
		} else if (tries < 5) {
		...
		for (var i = 0; i < letters.length; i++) {
			if (counter + space === letters.length) {
				...
				displayTries.innerHTML = "You Win! You the Rickeist Rick!";
			}
		}
	}
	...
	function check(keyPressed) {
		...
		if (tries === 0 || counter + space === letters.length) {
			...
			alert('You already guessed that letter. Try another newb.');
			return;
		}
		for (var i = 0; i < word.length; i++) {
	...
	function play() {
		randomWord = [
			["Birdperson", "Mr Poopybutthole", "", "Morty Jr", "Mr Meeseeks", "Squanchy", "Cronenbergs", "Unity", "Prince Nebulon", "Artricia", "Kyle", "Zeep Xanflorp"],
			["Rick", "Morty", "Summer", "Beth", "Jerry", "Jessica", "Evil Morty", "Jaguar", "Plumber Rick", "Cop Rick", "Lucy", "Doofus Rick", "Logic", "Rocko", "Diane Snachez", "Simple Rick", "The President", "Principal Vagina", "Hemorrhage"]
		];
